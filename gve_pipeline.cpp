...
namespace gve
{
	...
	void GvePipeline::createRenderPass()
	{
		VkAttachmentDescription colorAttachment{};
		colorAttachment.format = gveDevice.swapChainImageFormat();
	...
	void GvePipeline::createGraphicsPipeline(const std::string& vertFilePath, const std::string& fragFilePath, const PipelineConfigInfo& configInfo)
	{
		...
		viewport.x = 0.0f;
		viewport.y = 0.0f;
		viewport.width = (float)gveDevice.swapChainExtent().width;
		viewport.height = (float)gveDevice.swapChainExtent().height;
		viewport.minDepth = 0.0f;
		viewport.maxDepth = 1.0f;

		VkRect2D scissor{};
		scissor.offset = { 0, 0 };
		scissor.extent = gveDevice.swapChainExtent();
		...
		if(vkCreateGraphicsPipelines(gveDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo, nullptr, &graphicsPipeline) != VK_SUCCESS)
		{
