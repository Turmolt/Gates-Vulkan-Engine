...
namespace gve
{
	...
	VkResult GveSwapChain::submitCommandBuffers(const VkCommandBuffer* buffers, uint32_t* imageIndex)
	{
		...
		auto result = vkQueuePresentKHR(gveDevice.presentQueue(), &presentInfo);
	...
	{
		SwapChainSupportDetails swapChainSupport = gveDevice.getSwapChainSupport();
		...
		createInfo.surface = gveDevice.surface();
		...
		QueueFamilyIndices indices = gveDevice.findPhysicalQueueFamilies();
	...
	void GveSwapChain::createRenderPass()
	{
	...
	void GveSwapChain::createFramebuffers()
	{
	...
	void GveSwapChain::createDepthResources()
	{
	...
	VkFormat GveSwapChain::findDepthFormat()
	{
		return gveDevice.findSupportedFormat(
